...
static var bankBack : boolean = false;
...
function Start() {
    ...
    inboxPressed = true;
	is_updating = false;
	startX = Global.screenW * 0.3125; //was 125 for 3D
	startY = Global.screenH * 0.25; //was 45 for 3D	
	...
	swapAlertRect = Rect (Global.screenW * 0.3125, Global.screenH * 0.3125,Global.screenW * 0.4583, Global.screenH * 0.333);
}
...
// Global.screenW * 0.3125, Global.screenH * 0.3125,Global.screenW * 0.4583, Global.screenH * 0.333
function DrawInventory() {

	//goInvFrame.renderer.enabled = true;
	GUI.DrawTexture(Rect(Global.screenW * 0.22, Global.screenH * 0.15, Global.screenW * 0.78, Global.screenH * 0.85), texInventoryFrame,ScaleMode.StretchToFill,true,1);
...
function OnGUI()
{
	...
	if(bInventory)
	{
		...
		if(bIsSelItemHigh)
		{	
			GUI.DrawTexture(Rect(invWireFrame[selitem].x-Global.screenW * 0.00416,invWireFrame[selitem].y-Global.screenH * 0.00625,texwidth+Global.screenW * 0.0083,texheight+Global.screenH * 0.0125),texSelInvItem,ScaleMode.StretchToFill,true,0);		
			/*if(!(itemsInInventory[selitem].slot == 7) && !(itemsInInventory[selitem].slot == 8))
			{
				if(GUI.Button(Rect(Global.screenW * 0.3125, Global.screenH * 0.84375,Global.screenW * 0.1729, Global.screenH * 0.1125), "EQUIP", backbtnstyle))
				{
					if(!scriptBattle.bBattle)
					{
						EquipSelectedItem();
			...
			if(selstate)
			{
				...
				{
					invToolTipX = invWireFrame[selitem].x + paddingX + texwidth + Global.screenW * 0.0208;
					invToolTipY = startY;
				}
				else
				{
					invToolTipX = startX + (paddingX + texwidth)*((selitem % itemsPerRow)-Global.screenW * 0.00416);
					invToolTipY = startY;
				}
		...
		if(GUI.Button(Rect(Global.screenW * 0.28, Global.screenH * 0.85,Global.screenW * 0.14375, Global.screenH * 0.1), "VENDOR", btnstyle))
			{
				if (bIsSelItemHigh) 
					{
        ...
        if(GUI.Button(Rect(Global.screenW * 0.45, Global.screenH * 0.85,Global.screenW * 0.14375, Global.screenH * 0.1), "INBOX", bBlink ? inboxBtnStyle : inboxBtnStyleP))
		{   var bGUI;
            bBlink = true;
		...
		if(GUI.Button(Rect(Global.screenW * 0.78125, Global.screenH * 0.85,Global.screenW * 0.14375, Global.screenH * 0.1), "BACK", btnstyle))
		{
			bankBack = true;
		...
		if(GUI.Button(Rect(Global.screenW * 0.625, Global.screenH * 0.14375,Global.screenW * 0.25625, Global.screenH * 0.1125), "Transfer  to  inventory", btnstyle))
		{
			if (bIsSelItemHigh) {
				if (itemsInInventory[selitem].quantity > 1){
	...
	if(bPlayerEq)
	{
		...
		if(GUI.Button(Rect(Global.screenW * 0.3125, Global.screenH * 0.8437,Global.screenW * 0.1729, Global.screenH * 0.1125), "UNEQUIP", backbtnstyle))
		{
			if(bIsSelItemHigh)
			{
				UnEquipSelectedItem();
