...
//     mMode = MODE_INTRO;
	...
	if( mMode == MODE_CUTSCENES )
		{
			pos1 = Vector2( 0, 0 );
			pos2 = Vector2( 0, -Global.screenH );
			nrSlides = 3;
		}
	else if( mMode == MODE_INTRO )
		{
			pos1 = Vector2( 0, -Global.screenH * 1.043 );
			mSliding = true;
			activeSlide = 0;
			nrSlides = 0;
		}
...
{   
	...
	GUI.DrawTexture(Rect(0,0,Global.screenW,Global.screenH),blackText,ScaleMode.StretchToFill, true, 1);// de completat
	...
	if( !mSliding )
		{
			...
			GUI.DrawTexture( Rect( 0, 0, Global.screenW,Global.screenH ), tex );
		}
	else
		{
			...
			if( pos1.x > -Global.screenW )
				{
					pos1.x -= moveSpeed;
					tex1 = Resources.Load( "Animations/cutscenes/" + mMob + "/cut" + activeSlide + "low", Texture2D );
					GUI.DrawTexture( Rect( pos1.x, pos1.y, Global.screenW,Global.screenH ), tex1 );
				}
			else if( pos2.y < 0 )
				{
					...
					GUI.DrawTexture( Rect( pos2.x, pos2.y, Global.screenW,Global.screenH), tex1 );
				}
			else
				{
					mSliding = false;
