...
var dollarButtonStyle 	: GUIStyle;
var stButtonStyle 		: GUIStyle;
var waitButtonStyle		: GUIStyle;

static var variable : boolean = true;
var treiBut : boolean = true;
...
//var styleRunAwayBut			: GUIStyle;
//var styleInvBut				: GUIStyle;
//var styleSpecialAttack		: GUIStyle;
...
var nsToken 		: int;			// Variable for soul tokens
...
static var bNewSpecialLearned : int; 		//-1 lost ability, 0 no change, +1 gained ability
static var bShowNewSpecial : int;			// same as above
...
var diedTime : System.DateTime = new System.DateTime();
var CurrentTime : System.DateTime = new System.DateTime();
var deltaDeadTime : int = 0;
var continueToNextScene : boolean = false;
...
function SendInventoryItems()
{
	...
	for(var i = 0; i < Inventory.maxInvItems; i++ )
		if( Inventory.itemsInInventory[ i ].quantity != 0 && Inventory.itemsInInventory[ i ].id != 0 )
			k++;
	for( i = 0; i < Inventory.maxEquipedItems; i++ )
		if( Inventory.itemsEquiped[ i ].quantity != 0 && Inventory.itemsEquiped[ i ].id != 0 )
			k++;
	...
	for(i = 0; i < Inventory.maxInvItems; i++)
		{	
			if( Inventory.itemsInInventory[ i ].quantity != 0 || Inventory.itemsInInventory[ i ].id != 0 )
				{
					postData.AddField("" + k + "_item_id", Inventory.itemsInInventory[i].id );
		       		postData.AddField("" + k + "_quantity", Inventory.itemsInInventory[i].quantity );
		       		postData.AddField("" + k + "_active", Inventory.itemsInInventory[i].equipped );
		       		print("Inventory.itemsInInventory[i].equipped " + Inventory.itemsInInventory[i].equipped );
		       		postData.AddField("" + k + "_add_time", Inventory.itemsInInventory[i].temporar );
		       		++k;
				}
		}
	for(i = 0; i < Inventory.maxEquipedItems ; i++)
		{
			if( Inventory.itemsEquiped[ i ].quantity !=0 || Inventory.itemsEquiped[ i ].id != 0 )
				{
					postData.AddField("" + k + "_item_id", Inventory.itemsEquiped[i].id);
					print("--=--=-=---=--=----=------" + Inventory.itemsEquiped[i].id);
					postData.AddField("" + k + "_quantity", Inventory.itemsEquiped[i].quantity);
		       	 	postData.AddField("" + k + "_active", Inventory.itemsEquiped[i].equipped);
		       	 	postData.AddField("" + k + "_add_time", Inventory.itemsInInventory[i].temporar );
					++k;
				}
		}
...
{	 			
		...
		//SendInventoryItems();
		/*if (Global.switchDefTex) {
			GameObject.Find("Plane").renderer.material.mainTexture = Global.venueTexture;
		}*/
...
function Init( FirstInit : boolean )
{
	...
	//nPts = 4;
	lootAlertRect = Rect (Global.screenW * 0.3125 ,Global.screenH * 0.3125,Global.screenW * 0.4583 ,Global.screenH * 0.375);
		    ...
		    if(scriptMissions.clearingBuildings)
	           {
	               ...
	               print("Global.venueTexture    " + Global.venueTexture);      
							GameObject.Find( "Plane" ).renderer.material.mainTexture = Global.venueTexture;
	...
	switch(Global.enemyChar.mobModel){
		...
		case "ZombieCultist1":
			Global.mobIsRanged = false;
			Global.mobSpecialIsRanged = false;
			cMob.init("Mob","Animations/Mobs1.5/ZombieCultist1", Vector3(2.7, 0, -7), Vector3(0.65, 0.65, -0.65), Vector3(3.7, 0, -7), Vector3(-3, 0, -7), 							FLAG_MOB1);
		break;
		case "ZombieCultist2":
			Global.mobIsRanged = false;
			Global.mobSpecialIsRanged = false;
			cMob.init("Mob","Animations/Mobs1.5/ZombieCultist2", Vector3(2.7, 0, -7), Vector3(0.65, 0.65, -0.65), Vector3(3.7, 0, -7), Vector3(-3, 0, -7), 							FLAG_MOB1);
		break;
		case "ZombieBrown":
			Global.mobIsRanged = false;
			Global.mobSpecialIsRanged = false;
		...
		case "SuperZombie":
			Global.mobIsRanged = false;
			Global.mobSpecialIsRanged = false;
			cMob.init("Mob","Animations/Mobs1.5/ZombieSuper", Vector3(2.3, -0.2, -7), Vector3(140, 140, -140), Vector3(3.7, 0, -7), Vector3(-3, 0, -7), 							FLAG_MOB1);
	...
	switch(position)
		{
			case POS_MID:
				theStyle = guiStyle3;
				printText(Rect(Global.screenW * 0 ,Global.screenH * 0.3125,Global.screenW * 0.5 ,Global.screenH * 0.3125), theText);
			...
			case POS_MOB1:
				theStyle = guiStyle2;
				printText(Rect(Global.screenW * 0.5 ,Global.screenH * 0.3125,Global.screenW * 0.5 ,Global.screenH * 0.3125), theText);
				break;
		}
	switch(theText)
		{
			...
			case "Defeat":
				...
				//endFight2();
				if (Global.Hp_incercare < 2)
						Global.Hp_incercare = tempHP;
				endFightDeath();
				break;
		}
}
...
{
	printText(Rect(Global.screenW * 0.4166 ,Global.screenH * 0.3125,Global.screenW * 0.25 ,Global.screenH * 0.3125), theText);
}
...
function endFightDeath()
{
	...
	if(values[0].length > 3)
		...
		xpToLvL = 0;
	//nLootXP = xpToLvL * (0.50); //15% of xp to level up
}
...
deltaDeadTime = ((CurrentTime.Hour*100+CurrentTime.Minute)*100+CurrentTime.Second) - Global.timeOfDeath;
//Debug.Log("deltaDeadTime" + deltaDeadTime + " " + Global.timeOfDeath + " " + ((CurrentTime.Hour*100+CurrentTime.Minute)*100+CurrentTime.Second));	
	
	regenHp = Time.time;
	if ( regenHp-timp > 1 && tempHP > Global.myChar.HP && Global.myChar.HP >1)
		{
			//Global.myChar.HP = Global.myChar.HP + 5;
			timp = Time.time;
		}

if(!bLoading)
{
	if (!loading3Sprite.hidden) {
		loading3Sprite.hidden = true;
	}
	if(Queue.length > 0){
		if(act.fin)
			{
			...
			switch(act.action)
			{
				...
				case DIE:
					Global.myChar.playerZombie = 1;
							print("A MURIT PLAYERUL  " + Global.myChar.playerZombie);
				...
				{	
					if(act.owner == "cPC")
						{	//animToPlay = "death";
							...
							//player dies and becomes a zombie
							...
							cMob.init("Mob","Animations/Mobs1.5/Zombie 2 Body/Zombie 2", Vector3(-2.5, 0, -7), Vector3(0.65, 0.65, 0.65), Vector3(-3.5, 0, -7), Vector3(-2.5, 0, -7), 							FLAG_MOB1);
	...
	//Debug.Log("shortTime is " + PlayerPrefs.GetInt("shortTime"));
	if (PlayerPrefs.GetInt("shortTime") == 0 && deltaDeadTime > 15 + Global.myChar.LVL && playerIsDead) {
		continueToNextScene = true;
	} else {
		if (PlayerPrefs.GetInt("shortTime") == 1 && deltaDeadTime > 15 + Global.myChar.LVL && playerIsDead) {
			continueToNextScene = true;
		}
	}
	
	if (continueToNextScene && PlayerPrefs.GetInt("shortTime") == 0) {							
		bDeathPenalty = false;
		nLootXP = 0;
		LoadSceneScript.MoreMobsToFight = false;
		endFight2();
	} else {
		if (continueToNextScene) {bDeathPenalty = false;
			LoadSceneScript.MoreMobsToFight = false;
			endFight2();
			//red = true;
		}		
...
//Zombification fade effect

function fadeOut()
{	
	/*var player : Transform = GameObject.FindWithTag("Player").transform;
	while(player.renderer.material.color.a > 0)
			{
				player.renderer.material.color.a -= 0.09f;
...
/*function ifredeem()
{
						...
						GUI.Label( Rect(Global.screenW * 0.2333 ,Global.screenH * 0.1781,Global.screenW * 0.5333 ,Global.screenH * 0.78125), "", generalFrame );
						GUI.Label( Rect( Global.screenW * 0.3291 ,Global.screenH * 0.1875,Global.screenW * 0.41666 ,Global.screenH * 0.09375 ), "Redeem!", guiStyle );
						guiStyle.normal.textColor = Color(1, 1, 1, 1);
				// Text and values		
						GUI.Label(Rect(Global.screenW * 0.3125 ,Global.screenH * 0.30625,Global.screenW * 0.41666 ,Global.screenH * 0.3125), "Redeem yourself", guiStyle);
						GUI.Label(Rect(Global.screenW * 0.4895 ,Global.screenH * 0.30625,Global.screenW * 0.40625 ,Global.screenH * 0.3125), sLootName, lootStyle);
						
						GUI.Label(Rect(Global.screenW * 0.2604 ,Global.screenH * 0.40625,Global.screenW * 0.625 ,Global.screenH * 0.3125), "Decrease zomfication by half with $", guiStyle);
						//GUI.Label(Rect(230, 130, 300, 100), "" + nLootGold, lootStyle);
						
						//GUI.Label(Rect(150, 164, 300, 100), "XP", guiStyle);
						//GUI.Label(Rect(230, 164, 300, 100), "" + nLootXP, lootStyle);
						
						GUI.Label(Rect(Global.screenW * 0.2604 ,Global.screenH * 0.61875,Global.screenW * 0.625 ,Global.screenH * 0.3125), "Become human with SoulTokens", guiStyle);
						//GUI.Label(Rect(230, 198, 300, 100), "" + nsToken, lootStyle);
					
						//if( Global.myChar.Money + nLootGold >= 0 )
						
							if( GUI.Button( Rect( Global.screenW * 0.427 ,Global.screenH * 0.78125,Global.screenW * 0.1145 ,Global.screenH * 0.0875 ), "Pay $", styleOrangeBut ) )
								{
									//bDeathPenalty = false;
									nLootXP = 0;
									LoadSceneScript.MoreMobsToFight = false;
						...
						//if(Global.myChar.soulToken >= 0)		
							if( GUI.Button( Rect( Global.screenW * 0.5625 ,Global.screenH * 0.78125,Global.screenW * 0.1145 ,Global.screenH * 0.0875), "Pay ST", styleOrangeBut ) )
								{
									//bDeathPenalty = false;
									nsToken = 0;
									LoadSceneScript.MoreMobsToFight = false;
									endFight2();
									//Global.playerZombie = false;
								}
								Global.playerZombie = false;
}
...
*/
function runAwayButtonDelegate(sender : UIToggleButton, val : boolean	)
{
	if(  Queue.length == 0 )
	{
		Global.Hp_incercare = Global.myChar.HP;
...
function inventoryButtonDelegate(sender : UIToggleButton, val : boolean	)
{		
		...
		//variable = false;
								...
								runAwayButton.hidden = true;
								bottomBarSprite	.hidden = true;	
					...
					///if(GUI.Button(Rect(Global.screenW * 0.7166 ,Global.screenH * 0.7718,Global.screenW * 0.1708 ,Global.screenH * 0.1125), "", AttributeStyle))
						///{
						//	atribute = true;
						//	menuInventory = true;
						//}
...
function specialButtonDelegate(sender : UIToggleButton, val : boolean	)
{
	selectedSpecialAttack = -1;
	doSpecial = !doSpecial;	
	bAttack = false;
...
function basicButtonDelegate(sender : UIToggleButton, val : boolean	)
{
	bAttack = !bAttack;
	doSpecial = false;
	menuInventory = false;
...
var basicButton		: UIToggleButton;
var specialButton	: UIToggleButton;
var inventoryButton	: UIToggleButton;
var runAwayButton	: UIToggleButton;

var bottomBarSprite		: UISprite;
var loading1Sprite		: UISprite;
var loading2Sprite		: UISprite;
var loading3Sprite		: UISprite;
var deathSprite			: UISprite;

function showAllButtons()
{
	 
	basicButton = UIToggleButton.create(mixToolkit, "BASIC.png","BASIC.png","DOWN.png",0,0);
	basicButton.setSize(Global.screenW * 0.14375 ,Global.screenH * 0.09375); 
	basicButton.position = Vector3(Global.screenW * 0.0875, -Global.screenH * 0.8875,-1);
	basicButton.onToggle += basicButtonDelegate;
	basicButton.hidden = true;
	
	specialButton = UIToggleButton.create(mixToolkit, "SPECIAL.png","SPECIAL.png","DOWN.png",0,0);
	specialButton.setSize(Global.screenW * 0.14375 ,Global.screenH * 0.09375); 
	specialButton.position = Vector3(Global.screenW * 0.3229, -Global.screenH * 0.8875,-1);
	specialButton.onToggle += specialButtonDelegate;
	specialButton.hidden = true;
	
	inventoryButton = UIToggleButton.create(mixToolkit, "INVENTORY.png","INVENTORY.png","DOWN.png",0,0);
	inventoryButton.setSize(Global.screenW * 0.14375 ,Global.screenH * 0.09375); 
	inventoryButton.position = Vector3(Global.screenW * 0.5416, -Global.screenH * 0.8875,-1);
	inventoryButton.onToggle += inventoryButtonDelegate;
	inventoryButton.hidden = true;
	
	runAwayButton = UIToggleButton.create(mixToolkit, "RUNAWAY.png","RUNAWAY.png","DOWN.png",0,0);
	runAwayButton.setSize(Global.screenW * 0.14375 ,Global.screenH * 0.09375); 
	runAwayButton.position = Vector3(Global.screenW * 0.75625, -Global.screenH * 0.8875,-1);
	runAwayButton.onToggle += runAwayButtonDelegate;
	runAwayButton.hidden = true;
...
{	 
	...
	bottomBarSprite = mixToolkit2.addSprite("combatbottom bar.png",0,0,5);
	bottomBarSprite.setSize (Global.screenW, Global.screenH * 0.1343);
	bottomBarSprite.position = Vector3(0, -Global.screenH * 0.8656);
	bottomBarSprite.hidden = true;
	
	loading1Sprite = mixToolkit4.addSprite("Loading1.png",0,0,5);
	loading1Sprite.setSize (Global.screenW,Global.screenH);
	loading1Sprite.position = Vector3(0,0);
	loading1Sprite.hidden = true;
	
	loading2Sprite = mixToolkit4.addSprite("Loading2.png",0,0,5);
	loading2Sprite.setSize (Global.screenW,Global.screenH);
	loading2Sprite.position = Vector3(0,0);
	loading2Sprite.hidden = true;
	
	loading3Sprite = mixToolkit4.addSprite("Loading3.png",0,0,5);
	loading3Sprite.setSize (Global.screenW,Global.screenH);
	loading3Sprite.position = Vector3(0,0);
	loading3Sprite.hidden = true;	
	
	
	deathSprite = mixToolkit5.addSprite("death.png",0,0,5);
	deathSprite.setSize (Global.screenW,Global.screenH);
	deathSprite.position = Vector3(0,0);
	deathSprite.hidden = true;
	
}

function substractOneSoulToken()
{	
			//print("Extrage un soulToken===========1");	
			var the_url = Global.server + "/mmo_iphone/soultokenback.php?userid=" + Global.myChar.id;
			//print("Extrage un soulToken===========2");	
            var postData : WWWForm = new WWWForm();	//print("Extrage un soulToken===========3");	
...
function OnGUI()
{
//		GUI.Label(Rect(20,50,100,100),guiText.text);
	
	 
	
		if (atribute)
			{
				GUI.Box(Rect(Global.screenW * 0.34375 ,Global.screenH * 0.09375,Global.screenW * 0.3125 ,Global.screenH * 0.75), "");
				GUI.BeginGroup(Rect(Global.screenW * 0.34375 ,Global.screenH * 0.0468,Global.screenW * 0.375 ,Global.screenH * 0.78125)); 
				GUI.Label(Rect(Global.screenW * 0.0208 ,Global.screenH * 0.0468,Global.screenW * 0.3958 ,Global.screenH * 0.0625), Global.myChar.Nick + " Level:" + Global.myChar.LVL);
				GUI.Label(Rect(Global.screenW * 0.0208 ,Global.screenH * 0.1406,Global.screenW * 0.3958 ,Global.screenH * 0.0625), Global.myChar.BRT + " Brutality");
				GUI.Label(Rect(Global.screenW * 0.0208 ,Global.screenH * 0.2031,Global.screenW * 0.3958 ,Global.screenH * 0.0625), Global.myChar.ACC + " Accuracy");
				GUI.Label(Rect(Global.screenW * 0.0208 ,Global.screenH * 0.2656,Global.screenW * 0.3958 ,Global.screenH * 0.0625), Global.myChar.FORT + " Fortitude");
				GUI.Label(Rect(Global.screenW * 0.0208 ,Global.screenH * 0.3281,Global.screenW * 0.3958 ,Global.screenH * 0.0625), Global.myChar.DEF + " Defense");
				GUI.Label(Rect(Global.screenW * 0.0208 ,Global.screenH * 0.3906,Global.screenW * 0.3958 ,Global.screenH * 0.0625), Global.myChar.HP + " Health");
				GUI.Label(Rect(Global.screenW * 0.0208 ,Global.screenH * 0.4531,Global.screenW * 0.3958 ,Global.screenH * 0.0625), Global.myChar.ENRG + " Energy");
				GUI.Label(Rect(Global.screenW * 0.0208 ,Global.screenH * 0.5156,Global.screenW * 0.3958 ,Global.screenH * 0.0625), Global.myChar.ATK + " attack power");
				GUI.Label(Rect(Global.screenW * 0.0208 ,Global.screenH * 0.5781,Global.screenW * 0.3958 ,Global.screenH * 0.0625), Global.myChar.EVASION + "% chance to dodge");
			
				if(GUI.Button(Rect(Global.screenW * 0.0208 ,Global.screenH * 0.6875,Global.screenW * 0.2708 ,Global.screenH * 0.0625), "Close"))
					{
						atribute = false;
					}
				GUI.EndGroup();
	...
	if( scriptGUICutScene.cutSceneActive ) 
			return;
	...
	if(bLoading)
		{
		    if(Global.randomNumber <= 0.33)
					loading3Sprite.hidden = false;
	        else 
	        	if(Global.randomNumber > 0.33 && Global.randomNumber <= 0.66)
		            loading3Sprite.hidden = false;
           		 else 
            		loading3Sprite.hidden = false;
					return;
	...
	if(bLevelUp != 0)
		{		 
			...
			//GUI.Label(Rect(Global.screenW * 0.3229 ,Global.screenH * 0.3,Global.screenW * 0.0833 ,Global.screenH * 0.09375), ((Global.myChar.BRT - nBRT) > 0 ? "+" : "") + (Global.myChar.BRT - nBRT), guiStyle3);
			//GUI.Label(Rect(Global.screenW * 0.3229 ,Global.screenH * 0.4,Global.screenW * 0.0833 ,Global.screenH * 0.09375), ((Global.myChar.ACC - nACC) > 0 ? "+" : "") + (Global.myChar.ACC - nACC), guiStyle3);
			//GUI.Label(Rect(Global.screenW * 0.3229 ,Global.screenH * 0.5062,Global.screenW * 0.0833 ,Global.screenH * 0.09375), ((Global.myChar.FORT - nFORT) > 0 ? "+" : "") + (Global.myChar.FORT - nFORT), guiStyle3);
			//GUI.Label(Rect(Global.screenW * 0.3229 ,Global.screenH * 0.6031,Global.screenW * 0.0833 ,Global.screenH * 0.09375), ((Global.myChar.DEF - nDEF) > 0 ? "+" : "") + (Global.myChar.DEF - nDEF), guiStyle3);
			
			GUI.Label(Rect(Global.screenW * 0.3229 ,Global.screenH * 0.3,Global.screenW * 0.0833 ,Global.screenH * 0.09375), System.Convert.ToString(Global.myChar.BRT), guiStyle3);
			GUI.Label(Rect(Global.screenW * 0.3229 ,Global.screenH * 0.4,Global.screenW * 0.0833 ,Global.screenH * 0.09375), System.Convert.ToString(Global.myChar.ACC), guiStyle3);
			GUI.Label(Rect(Global.screenW * 0.3229 ,Global.screenH * 0.5062,Global.screenW * 0.0833 ,Global.screenH * 0.09375), System.Convert.ToString(Global.myChar.FORT), guiStyle3);
			GUI.Label(Rect(Global.screenW * 0.3229 ,Global.screenH * 0.6031,Global.screenW * 0.0833 ,Global.screenH * 0.09375), System.Convert.ToString(Global.myChar.DEF), guiStyle3);
			scriptInventory.AddItemStats();
	...
	if(bShowNewSpecial != 0)
		{	 
			...
			if( bShowNewSpecial == -1 )
				...
				GUI.DrawTexture(Rect(0, 0, Global.screenW * 0.5541 ,Global.screenH * 0.9031), texNewSkill, ScaleMode.StretchToFill, true, 1);
			...
			if( bShowNewSpecial == 1 )
				{ 
					GUI.Label(Rect(Global.screenW * 0.0104 ,Global.screenH * 0.09375,Global.screenW * 0.54375 ,Global.screenH * 0.15625), "Congratulations!", lvlUpLabel);
					GUI.Label(Rect(Global.screenW * 0.0104 ,Global.screenH * 0.1875,Global.screenW * 0.54375 ,Global.screenH * 0.15625), "You learned a new skill:", lvlUpLabel);
				}
			else
				{
					GUI.Label(Rect(Global.screenW * 0.0104 ,Global.screenH * 0.09375,Global.screenW * 0.54375 ,Global.screenH * 0.15625), "Sorry!", lvlUpLabel);
					GUI.Label(Rect(Global.screenW * 0.0104 ,Global.screenH * 0.1875,Global.screenW * 0.54375 ,Global.screenH * 0.15625), "You lost a skill:", lvlUpLabel);
				}
			
			lvlUpLabel.normal.textColor = Color.green;
			GUI.Label(Rect(Global.screenW * 0.0104 ,Global.screenH * 0.3125,Global.screenW * 0.54375 ,Global.screenH * 0.15625), newSpecialLearned, lvlUpLabel);
		...
		if(bLooting )
			{                                                                                                                                                                                                                                      
				GUI.DrawTexture (Rect(Global.screenW * 0.2033 ,Global.screenH * 0.1781,Global.screenW * 0.5333 ,Global.screenH * 0.6437 ), texLootB, ScaleMode.StretchToFill, true, 1);
				GUI.DrawTexture (Rect(Global.screenW * 0.4375 ,Global.screenH * 0.3125,Global.screenW * 0.0375 ,Global.screenH * 0.05625), texLootIcon, ScaleMode.StretchToFill, true, 1);
				guiStyle.normal.textColor = Color(1, 1, 1, 1); 
				 
				GUI.Label(Rect(Global.screenW * 0.3125 ,Global.screenH * 0.30625,Global.screenW * 0.25 ,Global.screenH * 0.3125 ), "Loot", guiStyle);
				GUI.Label(Rect(Global.screenW * 0.4895 ,Global.screenH * 0.30625,Global.screenW * 0.2708 ,Global.screenH * 0.3125 ), sLootName, lootStyle);
				GUI.Label(Rect(Global.screenW * 0.3125 ,Global.screenH * 0.40625,Global.screenW * 0.625 ,Global.screenH * 0.3125), "$", guiStyle);
				GUI.Label(Rect(Global.screenW * 0.4375 ,Global.screenH * 0.40625,Global.screenW * 0.625 ,Global.screenH * 0.3125 ), "" + nLootGold, lootStyle);
				GUI.Label(Rect(Global.screenW * 0.3125 ,Global.screenH * 0.5156,Global.screenW * 0.625 ,Global.screenH * 0.3125 ), "XP", guiStyle);
				GUI.Label(Rect(Global.screenW * 0.4375 ,Global.screenH * 0.5156,Global.screenW * 0.625 ,Global.screenH * 0.3125 ), "" + nLootXP, lootStyle);
		        //PREACHERMAN animation begins 
		        // Global.screenW * 0.49458 ,Global.screenH * 0.625,Global.screenW * 0.1729 ,Global.screenH * 0.1031
				if(GUI.Button(Rect(Global.screenW * 0.49458 ,Global.screenH * 0.625,Global.screenW * 0.1729 ,Global.screenH * 0.1031), "Continue", styleOrangeBut))
					{
						if( scriptGUICutScene.HasCutScene( Global.enemyChar.mobname ) )
	...
	//Frame of the message window
		...
		deathSprite.hidden = false;
			//GUI.Label( Rect(112, 57, 256, 250), "", generalFrame );
			//GUI.Label( Rect( 158, 60, 200, 30 ), "Defeat!", guiStyle );
			guiStyle.normal.textColor = Color(1, 1, 1, 1);
	// Text and values		
		/*	GUI.Label(Rect(150, 98, 200, 100), "Choose!", guiStyle);
			GUI.Label(Rect(235, 98, 130, 100), sLootName, lootStyle);
			
			GUI.Label(Rect(150, 130, 300, 100), "Stay Zombie", guiStyle);
			//GUI.Label(Rect(230, 130, 300, 100), "" + nLootGold, lootStyle);
			
			GUI.Label(Rect(150, 164, 300, 100), "Pay $", guiStyle);
			//GUI.Label(Rect(230, 164, 300, 100), "" + nLootXP, lootStyle);
			
			GUI.Label(Rect(150, 198, 300, 100), "Pay SoulToken", guiStyle);
			//GUI.Label(Rect(230, 198, 300, 100), "" + nsToken, lootStyle);*/
			
		//Buttons of the defeat screen	
		/*	if( GUI.Button( Rect( 130, 250, 40, 28 ), "XP", styleOrangeBut ) )
				{
					bDeathPenalty = false;
					nLootGold = 0;
					LoadSceneScript.MoreMobsToFight = false;
...
//			Global.screenW * 0.49458 ,Global.screenH * 0.625,Global.screenW * 0.1729 ,Global.screenH * 0.1031 
	...
	GUI.DrawTexture(Rect(Global.screenW * 0.25 ,Global.screenH * 0.12,Global.screenW * 0.53 ,Global.screenH * 0.75), ceva);
	//if(drawLabel == true)
	//{
		//pivotPoint = Vector2(Screen.width * 0.6,Screen.height * 0.1);
	  //  GUIUtility.RotateAroundPivot (rotAngle, pivotPoint); 
	    
	       
		
		GUI.Label(Rect(Global.screenW * 0.8,Global.screenH * 0.15,Global.screenW * 0.20833 ,Global.screenH * 0.3125),System.Convert.ToString(Global.varTime - deltaDeadTime) + " seconds to respawn");
		//rotAngle += 270;	
	//}
	...
	//{
		...
		if( GUI.Button( Rect( Global.screenW * 0.85 ,Global.screenH * 0.2125,Global.screenW * 0.080 ,Global.screenH * 0.16 ),"",waitButtonStyle ) )
			{							
						//var timerString : String = System.Convert.ToString(deltaDeadTime);
						//print(timerString);
						
						 Global.varTime = 15 + Global.myChar.LVL;
						diedTime = System.DateTime.Now;
						Global.timeOfDeath = (diedTime.Hour*100+diedTime.Minute)*100+diedTime.Second;					
						PlayerPrefs.SetInt("deathTime", Global.timeOfDeath);
						PlayerPrefs.SetInt("shortTime", 0);
						playerIsDead = true;	
						//treiBut = false;								
			}
			 //Global.screenW * 0.4583 ,Global.screenH * 0.8125,Global.screenW * 0.0833 ,Global.screenH * 0.0875
		if(Global.myChar.soulToken >= 0)		
			if( GUI.Button( Rect( Global.screenW * 0.85 ,Global.screenH * 0.4125,Global.screenW * 0.080 ,Global.screenH * 0.16 ), "", stButtonStyle ) )
				{												
					bDeathPenalty = false;
					/*nsToken = 0;*/
				//	if (Global.myChar.soulToken > 0) {
						LoadSceneScript.MoreMobsToFight = false;					
						substractOneSoulToken();
						
						print("Extrage un soulToken===========");				
						Global.playerZombie = false;
						playerIsDead = true;
						///treiBut = false;
						endFight2();
		...
		if(Global.myChar.Money >= 20)
			if( GUI.Button( Rect( Global.screenW * 0.85 ,Global.screenH * 0.6125,Global.screenW * 0.080 ,Global.screenH * 0.16 ), "", dollarButtonStyle ) )
			{	//Global.varTime = 250;
				//PlayerPrefs.SetInt("shortTime", 1);
				//diedTime = System.DateTime.Now;
				//Global.timeOfDeath = (diedTime.Hour*100+diedTime.Minute)*100+diedTime.Second;					
				//PlayerPrefs.SetInt("deathTime", Global.timeOfDeath);
				dollarDown = true;
				Global.save_statsSlashTwo();
				playerIsDead = true;
				//treiBut = false;
				bDeathPenalty = false;
				endFight2();									
	...
	if( !bLooting && /*bLevelUp==0 &&*/ bShowNewSpecial == 0 && !bDeathPenalty )
		...
		//	GUI.DrawTexture(Rect(5, 3, 2, 15), texHPc1, ScaleMode.StretchToFill, true, 1);
			GUI.DrawTexture(Rect(Global.screenW * 0.0145 ,Global.screenH * 0.009375, ((cPC.HP+0.0)/tempHP)*Global.screenW * 0.4166 ,Global.screenH * 0.0468), texHPc2, ScaleMode.StretchToFill, true, 1);
			//GUI.DrawTexture(Rect(7+((cPC.HP+0.0)/tempHP)*200, 3, 2, 15), texHPc3, ScaleMode.StretchToFill, true, 1);
			GUI.Label(Rect(Global.screenW * 0.01041 ,Global.screenH * 0.0125,Global.screenW * 0.4375 ,Global.screenH * 0.0625),cPC.HP+"/" +tempHP, statsStyle); 
			
			// Mana char
			//GUI.DrawTexture(Rect(5, 20, 3, 13), texMana1, ScaleMode.StretchToFill, true, 1);
			GUI.DrawTexture(Rect(Global.screenW * 0.01666 ,Global.screenH * 0.0625, ((cPC.ENRG+0.0)/tempENRG)*Global.screenW * 0.2083 ,Global.screenH * 0.0406), texMana2, ScaleMode.StretchToFill, true, 1);
		//	GUI.DrawTexture(Rect(8+((cPC.ENRG+0.0)/tempENRG)*100, 20, 3, 13), texMana3, ScaleMode.StretchToFill, true, 1);
			GUI.Label(Rect(Global.screenW * 0.01041 ,Global.screenH * 0.0625,Global.screenW * 0.2291 ,Global.screenH * 0.0625),cPC.ENRG+"/"+tempENRG, statsStyle); 
			
			// HP mob
			//GUI.DrawTexture(Rect(473, 3, 2, 15), texHPm3, ScaleMode.StretchToFill, true, 1);
			GUI.DrawTexture(Rect(Global.screenW * 0.9833-((cMob.HP+0.0)/Global.enemyChar.HP)*Global.screenW * 0.4166, Global.screenH * 0.009375, ((cMob.HP+0.0)/Global.enemyChar.HP)*Global.screenW * 0.4166, Global.screenH * 0.04675), texHPm2,ScaleMode.StretchToFill, true, 1);
			//GUI.DrawTexture(Rect(471-((cMob.HP+0.0)/Global.enemyChar.HP)*200, 3, 2, 15), texHPm1, ScaleMode.StretchToFill, true, 1);
			GUI.Label(Rect(Global.screenW * 0.5645 ,Global.screenH * 0.0125,Global.screenW * 0.4375 ,Global.screenH * 0.0625),cMob.HP+"/"+Global.enemyChar.HP, statsStyle); 
			 
			// Mana mob
			//GUI.DrawTexture(Rect(472, 20, 3, 13), texMana3, ScaleMode.StretchToFill, true, 1);
			GUI.DrawTexture(Rect(Global.screenW * 0.9833-((cMob.ENRG+0.0)/Global.enemyChar.ENRG)*Global.screenW * 0.2083, Global.screenH * 0.0625, ((cMob.ENRG+0.0)/Global.enemyChar.ENRG)*Global.screenW * 0.2083, Global.screenH * 0.0406), texMana2,ScaleMode.StretchToFill, true, 1);
			//GUI.DrawTexture(Rect(469-((cMob.ENRG+0.0)/Global.enemyChar.ENRG)*100, 20, 3, 13), texMana1, ScaleMode.StretchToFill, true, 1);
			GUI.Label(Rect(Global.screenW * 0.775 ,Global.screenH * 0.0625,Global.screenW * 0.2291 ,Global.screenH * 0.0625),cMob.ENRG+"/"+Global.enemyChar.ENRG, statsStyle); 
			
			// Interface
			//GUI.DrawTexture(Rect(215, 5, 51, 39), texTV, ScaleMode.StretchToFill, true, 1);
			//Global.screenW * 0.5104 ,Global.screenH * 0.1875,Global.screenW * 0.2083 ,Global.screenH * 0.075
			
			GUI.Label(Rect(Global.screenW * 0.5104 ,Global.screenH * 0.1093,Global.screenW * 0.4791 ,Global.screenH * 0.075), Global.enemyChar.mobname, mobLabelStyle);
			GUI.Label(Rect(Global.screenW * 0.0104 ,Global.screenH * 0.1093,Global.screenW * 0.4791 ,Global.screenH * 0.075), Global.myChar.Nick, charLabelStyle);
			guiStyle.normal.textColor = Color(1, 1, 1, 1);
			GUI.Label(Rect(Global.screenW * 0.0104, Global.screenH * 0.1875,Global.screenW * 0.2083 ,Global.screenH * 0.075), "" + Global.myChar.LVL, charLabelStyle);
			GUI.Label(Rect(Global.screenW * 0.78125, Global.screenH * 0.1875,Global.screenW * 0.2083 ,Global.screenH * 0.075), "" + cMob.LVL, mobLabelStyle);
		} 
		
	//draw bottom-bar if inventory not opened
	if(!Inventory.bInventory && !bLooting /*&& bLevelUp==0*/ && bShowNewSpecial == 0 && !bDeathPenalty)
	{
		bottomBarSprite.hidden = false;
		...
		basicButton.hidden = false;
		
		
		
		/*if(GUI.Button(Rect(42, 284, 69, 30), "", style))
			{
				bAttack = !bAttack;
				doSpecial = false;
				menuInventory = true;
		...
		if(bAttack && Queue.length == 0 && cPC.HP > 0 && cMob.HP > 0)
		{
			...
			if(GUI.Button(Rect(Global.screenW * 0.70, Global.screenH * 0.2943,Global.screenW * 0.1729, Global.screenH * 0.2305), "", style) && (Queue.length == 0))
			{				
				bAttack = false;
				if(cPC.WepType == 3)
					{
			...
			if(playerHasBrace > -1)
				{
					...
					if(GUI.Button(Rect(Global.screenW * 0.0875, Global.screenH * 0.3906,Global.screenW * 0.1729 ,Global.screenH * 0.1156), "", style) && (Queue.length == 0))
						{
			...
			if(playerHasConcentrate > -1){
				...
				if(GUI.Button(Rect(Global.screenW * 0.0875, Global.screenH * 0.5468,Global.screenW * 0.1729 ,Global.screenH * 0.1156), "", style) && (Queue.length == 0))
					{
			...
			if(playerHasRecover > -1)
				{
					...
					if(GUI.Button(Rect(Global.screenW * 0.0875, Global.screenH * 0.7031,Global.screenW * 0.1729 ,Global.screenH * 0.1156), "", style) && (Queue.length == 0))
						{
		...
		/*if(GUI.Button(Rect(155, 284, 69, 30), "", styleSpecialAttack))
			{
				selectedSpecialAttack = -1;
				doSpecial = !doSpecial;	
				bAttack = false;
		...
		{	
			selectedSpecialAttack = GUI.SelectionGrid(Rect(Global.screenW * 0.2916, Global.screenH * 0.2343, Global.screenW * 0.1708, playerSpecialAttacks.length * gridHSpecialAttacks), selectedSpecialAttack, sPlayerSpecialAttacks, 1, specialAttacksGridStyle);
			print ("Aici intra in special attack-------------");			
			for( var i = 0; i < playerSpecialAttacks.length; i++ )
				if( GUI.Button( Rect(Global.screenW * 0.2916, Global.screenH * 0.1875 + gridHSpecialAttacks/320 * Global.screenH * 0.00343 * i, Global.screenW * 0.25, gridHSpecialAttacks/320 *  Global.screenH),( " ( " + sPlayerSpecialAttacks[ i ] + " ) - " + cPC.SpecialAttacks[playerSpecialAttacks[i]].energcost  ), specialAttacksGridStyle ) )
					{	print ("Aici intra in special attack+++++++++++++++");
						selectedSpecialAttack = i;		print ("Aici intra in special attack");				
					}
		}
		...
		if((selectedSpecialAttack >-1)&&(doSpecial))
		{
			if(cPC.SpecialAttacks[playerSpecialAttacks[selectedSpecialAttack]].energcost > cPC.ENRG)
			...
			else
				{
					...
					theSwipe = new Swipe(cPC.SpecialAttacks[playerSpecialAttacks[selectedSpecialAttack]].attackname, swipeImage, swipeParticles,
		...
		inventoryButton.hidden = false;
		/*if(GUI.Button(Rect(260, 284, 69, 30), "", styleInvBut))
			{
...
*/
		...
		runAwayButton.hidden = false;
		/*if(GUI.Button(Rect(363, 284, 69, 30), "", styleRunAwayBut))
			{
				if(  Queue.length == 0 )
				{
					Global.Hp_incercare = Global.myChar.HP;
	...
	if(bReLoot && !bLootAlert && !scriptInventory.bInventory)
		{	
			if(GUI.Button(Rect(Global.screenW * 0.4145, Global.screenH * 0.625,Global.screenW * 0.1729 ,Global.screenH * 0.1031), "Loot", styleOrangeBut))
				{
					bReLoot = false;
					if(!scriptInventory.LootItem(idLootItem))
						{
...
function DoLootAlert (windowID : int) 
{
	...
	GUI.Label(Rect(Global.screenW * 0.0416, Global.screenH * 0.09375,Global.screenW * 0.3958 ,Global.screenH * 0.125),"Not enough free slots.\nPlease remove some items.");
	if(GUI.Button(Rect(Global.screenW * 0.1541, Global.screenH * 0.2187,Global.screenW * 0.1729 ,Global.screenH * 0.1125), "Close", scriptInventory.styleButSmll))
			bLootAlert = false;
	bReLoot = true;
}
