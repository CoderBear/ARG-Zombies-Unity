...
if(!bLoading)
{
	...
	if (PlayerPrefs.GetInt("shortTime") == 0 && deltaDeadTime > 500 && playerIsDead) {
		continueToNextScene = true;
	} else {
		if (PlayerPrefs.GetInt("shortTime") == 1 && deltaDeadTime > 250 && playerIsDead) {
			continueToNextScene = true;
		}
	}
...
function ifredeem()
{
						...
						GUI.Label( Rect( 158, 60, 200, 30 ), "Redeem!", guiStyle );
						guiStyle.normal.textColor = Color(1, 1, 1, 1);
				// Text and values		
						GUI.Label(Rect(150, 98, 200, 100), "Redeem yourself", guiStyle);
						GUI.Label(Rect(235, 98, 130, 100), sLootName, lootStyle);
						
						GUI.Label(Rect(125, 130, 300, 100), "Decrease zomfication by half with $", guiStyle);
						...
						GUI.Label(Rect(125, 198, 300, 100), "Become human with SoulTokens", guiStyle);
						//GUI.Label(Rect(230, 198, 300, 100), "" + nsToken, lootStyle);
					
						//if( Global.myChar.Money + nLootGold >= 0 )
							if( GUI.Button( Rect( 205, 250, 55, 28 ), "Pay $", styleOrangeBut ) )
								{
									//bDeathPenalty = false;
									nLootXP = 0;
									LoadSceneScript.MoreMobsToFight = false;
						...
						//if(Global.myChar.soulToken >= 0)		
							if( GUI.Button( Rect( 270, 250, 55, 28 ), "Pay ST", styleOrangeBut ) )
								{
									//bDeathPenalty = false;
									nsToken = 0;
									LoadSceneScript.MoreMobsToFight = false;
...
function showAllButtons()
{
	...
	basicButton.setSize(69, 30); 
	basicButton.position = Vector3(42, -284,-1);
	...
	specialButton.setSize(69, 30); 
	specialButton.position = Vector3(155, -284,-1);
	...
	inventoryButton.setSize(69, 30); 
	inventoryButton.position = Vector3(260, -284,-1);
	...
	runAwayButton.setSize(69, 30); 
	runAwayButton.position = Vector3(363, -284,-1);
...
//		GUI.Label(Rect(20,50,100,100),guiText.text);
	...
	// Text and values		
		...
		/*	if( GUI.Button( Rect( 130, 250, 40, 28 ), "XP", styleOrangeBut ) )
			...
			//if( Global.myChar.Money + nLootGold >= 0 )
		if( GUI.Button( Rect( 170, 260, 40, 28 ), "Wait", styleOrangeBut ) )
			{							
						//var timerString : String = System.Convert.ToString(deltaDeadTime);
						//print(timerString);
							GUI.Label(Rect(20,20,100,100),deltaDeadTime.ToString());
		...
		if(Global.myChar.soulToken >= 0)		
			if( GUI.Button( Rect( 220, 260, 40, 28 ), "ST", styleOrangeBut ) )
				{												
					/*bDeathPenalty = false;
					nsToken = 0;*/
					LoadSceneScript.MoreMobsToFight = false;
		...
		if(Global.myChar.playerZombie >= 0)
			if( GUI.Button( Rect( 270, 260, 40, 28 ), "$", styleOrangeBut ) )
			{
	...
	if(!Inventory.bInventory && !bLooting && bLevelUp==0 && bShowNewSpecial == 0 && !bDeathPenalty)
	{
		...
		if(doSpecial && Queue.length == 0 && cPC.HP > 0 && cMob.HP > 0)
		{
			...
			for( var i = 0; i < playerSpecialAttacks.length; i++ )
				if( GUI.Button( Rect( 140, 60 + gridHSpecialAttacks * 1.1 * i, 120, gridHSpecialAttacks ),( " ( " + sPlayerSpecialAttacks[ i ] + " ) - " + cPC.SpecialAttacks[playerSpecialAttacks[i]].energcost  ), specialAttacksGridStyle ) )
					{
