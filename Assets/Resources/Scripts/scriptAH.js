...
{
	if(aRect.yMin>35 && aRect.yMax<307) 
		return GUI.Button(aRect, aStr, styleList); 
	else return false;
}
	...
	if(nSellItem != -1)
		{
			...
			if(parseInt(sQuantity)>t.quantity) 
				sQuantity = ""+t.quantity;     
			sAux = sPrice;
			sAux = GUI.TextField (Rect (183, 218.5, 92.5, 20), sAux, 25);
...
function BuyItem()
{
	...
	is_updating_ah = true;	
	var download = new WWW(Global.server + "/mmo_iphone/auction_buy.php?id_user=" + nUserId +"&auction=" + AHItems[nBuyItem].id_auction);
	yield download;
...
function SellItem()
{
	...
	var download : WWW = new WWW(Global.server + "/mmo_iphone/auction.php?id_user=" + nUserId + "&id_item=" +
					t.id + "&quantity=" + sQuantity + "&price=" + sPrice +"&exp=" + sExpire);
	yield download;
