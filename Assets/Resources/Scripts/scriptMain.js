...
//static var tex: Texturi = new Texturi();
//tex.loadTextures();

// DELETE THIS BLASPHEMY
var nMobType : int = 1;

//var defaultLat = 44.411172;
//var defaultLon = 26.070458;
var defaultLat : float;
var defaultLon : float;
//var defaultLat = 44.415172;
//var defaultLon = 26.076158;

var lblStyle: GUIStyle = new GUIStyle();
lblStyle.normal.textColor = Color.black;
//lblStyle.normal.background = mainScript.tex.texSpecialQuad;
...
class mapZone{//patrat de pe harta 
	var lat		: float;
	var lon		: float;
	var zone	: Rect;
	var tip		: int;
...
function Awake(){
	...
	print("starting location service from start() 20 seconds timeout " );
	locating = false;
...
function loadSpecialZones(){
	...
	var values3 : String[];
	/*	
	var minLat: float = lat-dimKm*2;
	var maxLat: float = lat+dimKm*2;
	var minLon: float = lon-dimKm*2;
	var maxLon: float = lon+dimKm*2;
	*/
	...
	//Log.add("lat: " + lat + "  lon: " + lon);
	//Log.add("minLat: " + minLat + "  maxLat: " + maxLat + "minLon: " + minLon + "  maxLon: " + maxLon);
	...
	//print(specialZonesStr);
	//Log.add(specialZonesStr);
	//print(specialZonesStr);
...
var	the_url = Global.server + "/mmo_iphone/position1.php?lat1="+minLat+"&lat2="+maxLat+"&lon1="+minLon+"&lon2="+maxLon													+"&user_id="+Global.myChar.id;
	...
	if(nrOfPlayers > 0){
		...
		//Log.add("players in range:"+nrOfPlayers);
		for(i = 0; i < nrOfPlayers;i++){
			...
			//nearplayer.lat =Mathf.Abs(160 - (240-240*(lat-templat)/dimKm) - 240);
			//nearplayer.long =Mathf.Abs(180 - (240-240*(lon-templong)/dimKm) - 240);
          ...
          //  nearplayer.lat = parseFloat(tempdata[4]);
            //Debug.Log(" NearPlayer coords : long = "+parseFloat(tempdata[3])+" lat = "+parseFloat(tempdata[4]) );
            nearplayer.nick = tempdata[5];
			nearplayer.level = tempdata[6];
			players.Add(nearplayer);
			playersName[i] = nearplayer.nick;
			//Log.add(nearplayer.id + "face:"+nearplayer.avatarid+"at"+nearplayer.long+":"+nearplayer.lat);
			displayCount = 0;
...
function reLoadMap(){
	//---------google map
	//Log.add("getMap with lat: " + lat + " + lon: " + lon + " and " + zoom + " zoom.");
	//print("getMap with lat: " + lat + " + lon: " + lon + " and " + zoom + " zoom.");
...
function inviteTrade(response : int){
	...
	print("inviteTrade, ID = " + nUserID + ", to = " + sTo + ", resp = " + response);
...
function getUpdates(){
	...
	if(Time.timeSinceLevelLoad > lastTime + 5){
		...
		if(!download.error && download.text.length > 4){
			print("Result is: " + download.text);
			//print(values[1][0].ToString()+"TRADEEEEEeeee");
			values = Regex.Split(download.text, ";");
			bNewChat = values[0][3] == "1";
			if(values[1][0]=="1"){
				print("Trade response is 1.");       // tr 1 = trade accept
				sFrom = values[2];
				if(nTradeState == 4) nTradeState = 1; else{
					bNewTrade = true;
                    bTrade = true;
					nTradeState = 3;
				}
			}else if(values[1][0]=="2"){
				print("Trade response is 2.");       // tr 2 = trade cancel
				nTradeState = 2;
				bNewTrade = false;
			}
		}
...
{  // exitFromTrade = false;
    ...
    reloadPlayers = true;            
	UpdateLoop();
    Debug.Log(" clearing building is "+scriptMissions.clearingBuildings);
...
function Update(){
	...
	if ( iPhoneSettings.locationServiceStatus == LocationServiceStatus.Stopped ) {
		//print(" locationservice stopped,, trying to start from update " );
		startLocationService();		
	}
	if( !located && !locating )
		startLocationService();
...
function shouldGUI() : boolean{
	//return (bGUI&&(!Inventory.bInventory)&&(!Inventory.bPlayerEq)&&(!Craft.bCraft)&&(!scriptAH.bSell)													&&(!scriptAH.bBuy)&&(!scriptChat.bChat)&&(!scriptAH.bExpired)&&(!scriptTrade.bInitialized)&&(!bShowInfoScreen));
...
*/
    ...
    // name of char 
	if(!scriptTrade.bInitialized && !scriptAH.bSell && !scriptAH.bBuy &&!scriptAH.bExpired){
		GUI.Label(Rect(5, 230, 100, 40), Global.myChar.Nick, guiCharStyle);
	}
	// money and  level
	...
	if(Global.bDifficultySelected){
		//print("player is:"+nSelectedPlayer+"and is selected"+bSelectedPlayer);
		//print("FIGHT !?.");
	...
	if(bShowInfoScreen){
		...
		if(nrOfPlayers > 0)
		{
            ...
            if(nSelectedPlayer != -1 && bSelectedPlayer){
				...
				GUI.Label(Rect(278,50,80,20),""+nearPlayer.level, styleSelectedNearPlayer);
				styleSelectedNearPlayer.normal.textColor = Color(1, 1, 1, 1);
				GUI.Label(Rect(240,50,80,20),"Level: ",styleSelectedNearPlayer);
				styleSelectedNearPlayer.alignment = TextAnchor.MiddleCenter;
				if(GUI.Button(Rect(230, 65, 83,36),"Chat",styleButRedSmll)){
				...
				if(GUI.Button(Rect(230, 101, 83,36),"Trade",styleButRedSmll) ){
                   // exitFromTrade = false;
                   tradePressed = true;
					sTo = nearPlayer.nick;
					bAux = bTrade = !bTrade;
				...
				if(GUI.Button(Rect(230, 137, 83,36),"Duel",styleButBlueSmll)){
					if(bSelectedPlayer){
						Global.bDifficultySelected = true;
					}
				}
	...
	//Craft
	if(!Craft.bCraft){
		GUI.DrawTexture(Rect(388, 2, 82, 38), texButtonShadow, ScaleMode.StretchToFill, true, 1);
	}
	//Auction
	if(!bAH || scriptAH.bBuy || scriptAH.bSell || scriptAH.bExpired){
		GUI.DrawTexture(Rect(310, 2, 82, 38), texButtonShadow, ScaleMode.StretchToFill, true, 1);
	}
	//Chat
	if(!scriptChat.bChat){
		GUI.DrawTexture(Rect(232, 2, 82, 38), texButtonShadow, ScaleMode.StretchToFill, true, 1);
	}
	//Trade
	if(!bTrade){
	...
	//Craft
	if(GUI.Button(Rect(389, -2, 92, 31),"Craft", Craft.bCraft ? styleButP : styleBut)){
	...
	//Auction
	if(GUI.Button(Rect(311, -2, 92, 31),"Market", (bAH || scriptAH.bBuy || scriptAH.bSell || scriptAH.bExpired) ? styleButP : styleBut)){
		scriptMissions.InfoWindowActivated = false;
	...
	//Chat
	if(GUI.Button(Rect(233, -2, 92, 31),"Social",bSO ? styleButP : styleBut)){
     missionsClicked = false;
    bAux = bSO = !bSO;
		if(bAux){
			scriptMissions.InfoWindowActivated = false;
    ...
    */
	...
	//end Trade
    //Debug.Log("bTrade e "+bTrade +" nTradeState e " +nTradeState);
	if(bTrade){
		switch(nTradeState){
			case 0:
	...
	if(bCanShowBank)
	{
		...
		//  {
        	if(GUI.Button(Rect(155, -2, 92, 31), "Storage", IsHome() ? (theBank.bInventory ? styleButP : styleBut) : styleButGray)) {
			//bBank = !bBank;
        ...
        //   else if(GUI.Button(Rect(155, -2, 92, 31), "Storage", theBank.bInventory ? styleButGrayP : styleButGray)){
			...
			//if(!IsHome() && Inventory.bInventory)
			//{	//.Window (1, Rect (150, 100, 220, 120), DoMyWindow, "SALAM");
			//			GUI.Window(0, Rect(150,100,220,120),DoMyWindow,"sasa");
    ...
    if(GUI.Button(Rect(77, -2, 92, 31),"Missions", theMissions.getMissions() ? styleButP : styleBut) )
    {
        //exitFromTrade = true;
	...
	// Main Menu
	GUI.DrawTexture(Rect(-2, 2, 94, 38), texButtonShadow, ScaleMode.StretchToFill, true, 1);
	if(GUI.Button(Rect(-1,-2, 92, 31), "MAIN MENU", btnMMStyle)){
		//bLoadBattle = true;
		bMMenu = !bMMenu;
		Inventory.bPlayerEq =  false;
		Inventory.bInventory = false;
	...
	//Craft
	if(Craft.bCraft){
		GUI.DrawTexture(Rect(397, 4, 92, 45), texButtonShadow, ScaleMode.StretchToFill, true, 1);
		GUI.DrawTexture(Rect(397, -3, 85, 40), texMenuP, ScaleMode.StretchToFill, true, 1);
		GUI.Label(Rect(397,0,85,40),"Craft",styleButClear);	
	}
	//Auction
	if(bAH || scriptAH.bBuy || scriptAH.bSell || scriptAH.bExpired){
		GUI.DrawTexture(Rect(317, 4, 92, 45), texButtonShadow, ScaleMode.StretchToFill, true, 1);
		GUI.DrawTexture(Rect(317, -3, 85, 40), texMenuP, ScaleMode.StretchToFill, true, 1);
		GUI.Label(Rect(317,0,85,40),"Market",styleButClear);	
	}
	
	//Chat
	if(bSO ){
		GUI.DrawTexture(Rect(240, 4, 92, 45), texButtonShadow, ScaleMode.StretchToFill, true, 1);
		GUI.DrawTexture(Rect(240, -3, 85, 40), texMenuP, ScaleMode.StretchToFill, true, 1);
		GUI.Label(Rect(240,0,85,40),"Social",styleButClear);
        GUI.DrawTexture(Rect(214, 39, 77, 34), texButtonShadow, ScaleMode.StretchToFill, true, 1);
		GUI.DrawTexture(Rect(284, 39, 77, 34), texButtonShadow, ScaleMode.StretchToFill, true, 1);
        // social buttons 
        if(scriptChat.bChat|| bSO){
	 if(GUI.Button(Rect(215, 34, 70, 29),"Chat", (scriptChat.bChat || (bNewChat && bBlink)) ? styleButP : styleBut)){
		bAux = scriptChat.bChat = !scriptChat.bChat;
		if(bAux){
			bTrade = false;
			theMissions.setMissions( false );
	...
	//end Chat
        if(GUI.Button(Rect(285, 34, 70, 29),"Friends",   styleBut ))
        {
	...
	if(bCanShowBank && theBank.bInventory){
		GUI.DrawTexture(Rect(162, 5, 92, 45), texButtonShadow, ScaleMode.StretchToFill, true, 1);
		GUI.DrawTexture(Rect(162, -3, 85, 40), texMenuP, ScaleMode.StretchToFill, true, 1);
		GUI.Label(Rect(162,0,85,40),"Storage",styleButClear);
	}
	//end Bank
	
	//Missions
	if (theMissions.getMissions()){
	    GUI.DrawTexture(Rect(84, 5, 92, 45), texButtonShadow, ScaleMode.StretchToFill, true, 1);
	    GUI.DrawTexture(Rect(84, -3, 85, 40), texMenuP, ScaleMode.StretchToFill, true, 1);
	    GUI.Label(Rect(84,0,85,40),"Missions",styleButClear);
	}
	...
	if(bAH){
		//shadows
		GUI.DrawTexture(Rect(289, 39, 77, 34), texButtonShadow, ScaleMode.StretchToFill, true, 1);
		GUI.DrawTexture(Rect(359, 39, 77, 34), texButtonShadow, ScaleMode.StretchToFill, true, 1);
		GUI.DrawTexture(Rect(289, 69, 154, 35), texButtonShadow, ScaleMode.StretchToFill, true, 1);
        GUI.DrawTexture(Rect(289, 95, 154, 35), texButtonShadow, ScaleMode.StretchToFill, true, 1);
        
       
		//sell and buy buttons
		if(GUI.Button(Rect(360, 34, 70, 29),"Sell", scriptAH.bSell ? styleButP : styleBut)){
		...
		if(GUI.Button(Rect(290, 34, 70, 29),"Buy", scriptAH.bBuy ? styleButP : styleBut)){
        ...
        if(GUI.Button(Rect(290, 64, 140, 29),"Earn   $$", scriptAH.bBuy ? styleButP : styleBut)){
			
		}
        if(GUI.Button(Rect(290, 94, 140, 29),"Purchase   $$", scriptAH.bBuy ? styleButP : styleBut)){
					}
	...
	if (shouldGUI) {
		...
		//print( Global.myChar.EXP.ToString() + " " + xp_bar_multiplier + " " + xp_bar_height + "   " + Global.myChar.EXP + "  "  + xp_bar_height );
		...
		if (xp_bar_multiplier > 0) 
			if ( bBlink && bNewXP ) 
			{
				...
				//print("salam:  " + buffEXP + "  !!  " + Global.myChar.EXP);
			}
			else
...
function setHomebase2() 
{
	...
	while (download.error && download.error.ToString().Contains("Resolving host timed out"))
	{
   	...
   	if(download.error) {
		...
		if(wwwData.IndexOf("Homebase", 0) > 0)        // wwwData contine Home Updated! daca s-a updatat, daca nu il face int cu secundele de la ultima updatare
		{
...
function blinkXPbar()
{
	...
	{
		print("111: " + Global.myChar.EXP + "   " + Global.oldEXP );
	...
	if ( Global.myChar.EXP - Global.oldEXP < 0 )
	{
		print("222: " + Global.myChar.EXP + "   " + Global.oldEXP );
