	...
	for(i = 0; i<values2.length-1; i++)
		{
			if(parseInt(Regex.Split(values2[i], ";")[1]) == -1)
				...
				{
					download = new WWW(Global.server + "/mmo_iphone/item_info.php?id_item=" + parseInt(Regex.Split(values2[i], ";")[1]));
					yield download;
		...
		if(bMeReady && bYouReady && bFinalize)
			{
				...
				else if(upload.text.IndexOf("fail")>-1)
					{
						print("Trade failed.");
...
function GUIFrame()
{
	...
	switch(nGUIFrame)
	{
		...
		case GUI_INFO:
			...
			if(parseInt(sQuantity)>1) 
			GUI.Label(Rect(165, 160, 220, 26),"Trade pieces", styleText5);
			...
			if(parseInt(sQuantity)>myItems[nInvItem].quantity) 
...
function OnGUI()
{
	...
	if(GUI.Button(Rect(355, 274, 83, 36), "Cancel", styleBtnChat))
	    {
			bMeReady = false;
			bYouReady = false;
			bTrade = false;
