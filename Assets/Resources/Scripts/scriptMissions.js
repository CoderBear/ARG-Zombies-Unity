...
function OnGUI()
{
	...
	if (mMissions)      // window misiuni
	{
		if( cameraActive )
			{
				//scan qr code button;
				if( GUI.Button( Rect( 360, 275, 80, 25 ), "", styleScanBut ) )
					{
						ScanQR();
					}
				return;
			}
		GUI.DrawTexture(Rect(115,40,360,270), texMissionsWindow);
       ...
       /* if( GUI.Button( Rect( 370, 27, 70, 55 ), "Craft", chooseMission=="Craft" ? styleRedBut:styleRederBut ) )
		...
		if( GUI.Button( Rect( 180, 265, 69, 32 ), "", styleScanBut ) )
			{
				StartCamera();
				
			}
		if( GUI.Button( Rect( 270, 265, 69, 32 ), "", styleClear ) )
			{
				ClearBuilding();
			}
		if( GUI.Button( Rect( 360, 265, 69, 32 ), "", styleBack ) )
			{
				ExitMissions();
				backdown = true;
				//butDown = true;
		...
		if( iPhoneInput.touchCount > 0)
         ...
         if(!getBuildings)  
		         scrollPosition = GUI.BeginScrollView( scrollRect, scrollPosition, new Rect( 0, 0, 310, Global.missionsArray.length * 65 ) );
