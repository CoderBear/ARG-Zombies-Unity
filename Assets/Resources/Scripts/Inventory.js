...
function Start() 
{
	...
	is_updating = false;
	startX = 155; //was 125 for 3D
	startY = 90; //was 45 for 3D	
	texwidth = 52;
	texheight = 52;
	paddingX = 10;
	paddingY = 15;
...
{
	GUI.DrawTexture(Rect(startX-55, startY-70, 50 + itemsPerRow*(paddingX+texwidth), 120 + (maxInvItems/itemsPerRow)*(paddingY+texheight)), texInventoryFrame,ScaleMode.StretchToFill,true,1);
	...
	{	
		...
		GUI.DrawTexture(Rect(invWireFrame[i].x-30,invWireFrame[i].y-10,texwidth,texheight),texInvItems[i],ScaleMode.StretchToFill,true,1);
		if(itemsInInventory[i].quantity > 1)
		{
			if (itemsInInventory[i].quantity < 10) 
				...
				{
					GUI.DrawTexture(Rect(invWireFrame[i].x+3, invWireFrame[i].y+4, 1, 12), dark_pixel, ScaleMode.StretchToFill, true, 1);
				}
...
{
	GUI.DrawTexture(Rect(startX-35, startY-40, 40 + itemsPerRow*(paddingX+texwidth), 67 + (maxInvItems/itemsPerRow)*(paddingY+texheight)), texPEquipedFrame,ScaleMode.StretchToFill,true,0);
...
function OnGUI()
{
	...
	{
		if(GUI.Button(Rect(200,280,69,32),"",styleSendBut))
		{print("E aici2");
					if(selecteditem)
		...
		if(GUI.Button(Rect(120, 272, 83, 36), "VENDOR", btnstyle))
			{
				if (bIsSelItemHigh) 
					{
				...
				if(GUI.Button(Rect(210, 272, 83, 36),"BACK", btnstyle))
		{
		...
		if(scriptMain.bCanShowBank)
			if ( scriptMain.IsHome() )
				if(GUI.Button(Rect(300, 46, 123, 36), "Transfer  to  bank", btnstyle))
				{
					if (bIsSelItemHigh) 
					{
						if ( itemsInInventory[selitem].temporar == 0 )
		...
		if(scriptMain.bCanShowBank)
			if ( scriptMain.IsHome() )
				if( GUI.Button( Rect(230, 46, 50, 36), "< <", btnstyle) )
					{
						TransferAllToBank();
					}
	...
	if(bPlayerEq)
	{
		...
		if(bIsSelItemHigh)
		...
		else
			{
				countItemStats = 0;
				GUI.BeginGroup(Rect(285, 75, 180, 200)); 
				GUI.Label(Rect(45, 20 + countItemStats, 190, descLabelHeight), Global.myChar.Nick + " Level:" + Global.myChar.LVL);
				countItemStats += descLabelHeight;
				
				GUI.Label(Rect(45, 20 + countItemStats, 190, descLabelHeight), Global.myChar.BRT + " Brutality");
				countItemStats += descLabelHeight;
			
				GUI.Label(Rect(45 , 20 + countItemStats, 190, descLabelHeight), Global.myChar.ACC + " Accuracy");
				countItemStats += descLabelHeight;
				
				GUI.Label(Rect(45 , 20 + countItemStats, 190, descLabelHeight), Global.myChar.FORT + " Fortitude");
				countItemStats += descLabelHeight;
				
				GUI.Label(Rect(45 , 20 + countItemStats, 190, descLabelHeight), Global.myChar.DEF + " Defense");
				countItemStats += descLabelHeight;
				
				GUI.Label(Rect(45 , 20 + countItemStats, 190, descLabelHeight), Global.myChar.HP + " Health");
				countItemStats += descLabelHeight;
					
				GUI.Label(Rect(45 , 20 + countItemStats, 190, descLabelHeight), Global.myChar.ENRG + " Energy");
				countItemStats += descLabelHeight;
				
				GUI.Label(Rect(45 , 20 + countItemStats, 190, descLabelHeight), Global.myChar.ATK + " attack power");
				countItemStats += descLabelHeight;
					
				GUI.Label(Rect(45 , 20 + countItemStats, 190, descLabelHeight), Global.myChar.EVASION + "% dodge");
				countItemStats += descLabelHeight;
		...
		if ( selitem != -1 && selitem < itemsEquiped.length && itemsEquiped[selitem].id != 0 )
			{
				if(GUI.Button(Rect(150, 270, 83, 36), "", styleUnequipBut))
					{
						//if(bIsSelItemHigh)
						//{
							UnEquipSelectedItem();
				...
				if(GUI.Button(Rect(210, 270, 83, 36),"BACK", btnstyle))
		{
		...
		// Flag States
		if (GUI.Button(Rect(340, 47, 60, 36), "PVP", btnstyle))
			{
				script_main = GetComponent(scriptMain);
				scriptMain.FlagStateCheck = "PVP";
				script_main.FlagState();
			}
		if (GUI.Button(Rect(400,47, 60, 36), "Inactive", btnstyle))
			{
				script_main = GetComponent(scriptMain);
		...
		if(GUI.Button(Rect(400, 285, 69, 30), "", styleBackBut))
		{
			bPlayerEq = false;
			//if(bInvWasModified == true)
				//{
