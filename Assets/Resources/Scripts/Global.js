static var server : String = "http://www.argzombies.com";

//screen dimension variables
static var screenW : int;
static var screenH : int;
var getRes : boolean = true;
//end of screen dimension variables

var regenHp : double;
var timp : double; 

static var quitUpdateOK : boolean = false;
...
static var loadUserDataOnce : boolean = true;

static var StartEnergy : int = 0;
...
//foursquare stuff
var venues : Object[];
static var venuesTex : Texture2D[];
static var venueTexture : Texture2D;
static var switchDefTex : boolean = false;
...
function Start()
{
	...
	//scriptMarket.updateClaimTokensMessage();
	timp = Time.time;
	self = this;
    debugMode = Application.platform != RuntimePlatform.IPhonePlayer;
    timeOfDeath = PlayerPrefs.GetInt("deathTime");
  	if(((CurrentTime.Hour*100+CurrentTime.Minute)*100+CurrentTime.Second - PlayerPrefs.GetInt("deathTime"))<=varTime)
  	{
  		deathScreen = true;
  	}
  	venues = Resources.LoadAll("fsBacks", Texture2D);
  	venuesTex = new Texture2D[venues.length];
  	for (var i:int = 0; i<venues.length; i++) {
  		venuesTex[i] = venues[i];
  		venues[i] = null;
  	}
}
...
function Update()
{
	...
	if(getRes && Screen.width != screenW) {
		screenW = Screen.width;
		screenH = Screen.height;
		//getRes = false;
	}
    ...
    regenHp = Time.time;
	if ( regenHp-timp > 1 && Global.myChar.HP > Global.Hp_incercare && Global.Hp_incercare > 1)
		{
...
static function save_statsSlashTwo()
{	//------------------------
	//Implementare zombificare:
	//------------------------
			/*if (Global.playerZombie == true)
			...
			postData.AddField("brutality",Global.myChar.BRT+"");
			postData.AddField("accuracy", Global.myChar.ACC+"");
			postData.AddField("energy", (Global.StartEnergy/2)+"");
			postData.AddField("defense",Global.myChar.DEF+"");
			postData.AddField("fortitude",Global.myChar.FORT+"");
			//postData.AddField("hp", (Global.myChar.HP/2)+"");
			postData.AddField("hp", (Hp_incercare/2)+"");			
			postData.AddField("evasion", Global.myChar.EVASION+"");
			postData.AddField("attack", Global.myChar.ATK+"");
			postData.AddField("level", Global.myChar.LVL+"");
			postData.AddField("experience", Global.myChar.EXP+"");
...
static function getUserData()
{
	...
	{	
			...
			Global.myChar.soulToken = parseInt(values[25]);
			print("Nr de soulTokenuri este:         " + Global.myChar.soulToken);
			//Debug.Log("Intu de 25 " + values[25]);
...
//			Global.myChar.soulToken = parseInt(values[26]);
			...
			if ( Inventory.self != null )
				Inventory.self.AddItemStats();
			quitUpdateOK = true;
			Global.StartEnergy = Global.myChar.ENRG;
